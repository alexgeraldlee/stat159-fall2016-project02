...
best_pcr <- pcr_mod$coefficients[4:14,1,best_pcr_number]

#calculate the MSE using the test data
pcr_pred <- predict(pcr_mod, credit_test, ncomp = best_pcr)
...
pcr_final <- pcr(credit_y ~ credit_x, scale = FALSE, ncomp = best_pcr)

pcr_coefficients <- pcr_final$coefficients[3:13,1,13]

rdata_output <- '../../data/best-pcr.RData'
save(best_pcr, pcr_coefficients, pcr_mse)